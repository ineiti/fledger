...
pub async fn example() -> Result<(), JsValue> {
    ...
    /*
     * If negotiation is done, this closure will be called
     *
     */
    let ondatachannel_callback = Closure::wrap(Box::new(move |ev: RtcDataChannelEvent| {
        ...
        let onmessage_callback =
            Closure::wrap(
                Box::new(move |ev: MessageEvent| match ev.data().as_string() {
                    Some(message) => console_warn!("dc2 got: {:?}", message),
                    None => {}
                }) as Box<dyn FnMut(MessageEvent)>,
            );
     ...
     *
     */
    console_log!("pc1: start to receive - state {:?}", pc1.signaling_state());
    let mut answer_obj = RtcSessionDescriptionInit::new(RtcSdpType::Answer);
    answer_obj.sdp(&answer_sdp);
