...
use common::web_rtc::WebRTCCaller;
use common::web_rtc::WebRTCCallerState;
use common::web_rtc::WebRTCClient;use wasm_bindgen::JsValue;



pub async fn demo() -> Result<(), JsValue> {
    // Set up two PCs - one needs to have init == true, the other init == false
    let pc1 = WebRTCCaller::new(WebRTCCallerState::Initializer)?;
    let pc2 = WebRTCCaller::new(WebRTCCallerState::Follower)?;
    let mut rc1 = WebRTCClient::new(pc1);
    let mut rc2 = WebRTCClient::new(pc2);
