...
use web_sys::{
    console::log_1, MessageEvent, RtcDataChannel, RtcDataChannelEvent, RtcDataChannelState,
    RtcIceCandidate, RtcIceCandidateInit, RtcIceGatheringState, RtcPeerConnection,
...
impl WebRTCCallerWasm {
    ...
    async fn msg_receive(&mut self) -> Result<String, JsValue> {
        self.is_open().await?;
        for _ in 0..10 {
    ...
    /// an error is returned.
    async fn msg_send(&mut self, s: &str) -> Result<(), JsValue> {
        self.is_open().await?;
