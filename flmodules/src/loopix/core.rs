...
use crate::loopix::sphinx::{Sphinx, destination_address_from_node_id};
...
pub trait LoopixCore {
    ...
    fn create_sphinx_packet(&self, dest: NodeID, msg: NetworkWrapper, route: &[Node]) -> (Node, Sphinx) {
        // delays
    ...
    async fn create_route(&self, path_length: usize, start_provider: Option<NodeID>, dest_provider: Option<NodeID>, receiver: Option<NodeID>) -> Vec<Node> {
        ...
        if let Some(start_provider) = start_provider {
            ...
            let start_node = Node::new(NodeAddressBytes::from_bytes(start_provider.to_bytes()), *start_key);
            route.push(start_node);
        }
        // add mixnode route
        let mixnodes = self.get_storage().get_mixes().await;
        for i in 0..path_length {
            let mixnode = mixnodes[i as usize].choose(&mut rand::thread_rng()).unwrap();
        ...
        if let Some(receiver) = receiver {
            ...
            let receiver_node = Node::new(NodeAddressBytes::from_bytes(receiver.to_bytes()), *receiver_key);
...
mod tests {
    ...
    async fn setup() -> (MockLoopixCore, HashMap<NodeID, (PublicKey, StaticSecret)>, usize) {
        let path_length = 2;
        ...
        let config = LoopixConfig::default_with_path_length(LoopixRole::Mixnode, node_id, path_length, private_key.clone(), public_key.clone());

        let core = MockLoopixCore { config: config.core_config, storage: config.storage_config };

        core.get_storage().set_node_public_keys(node_public_keys).await;
    ...
    async fn test_create_route() {
        let (core, node_key_pairs, path_length) = setup().await;
        ...
        assert_eq!(route[0].address, NodeAddressBytes::from_bytes(our_id.to_bytes()));
    ...
    async fn test_sphinx_packet_simple() {
        ...
        assert_eq!(private_key.as_bytes(), node_key_pairs.get(&our_id).unwrap().1.as_bytes());

        // just create a packet with one layer of encryption
        let route = core.create_route(0, None, None, Some(our_id)).await;

        let msg = NetworkWrapper { module: MODULE_NAME.into(), msg: serde_yaml::to_string(&MessageType::Dummy).unwrap() };
        ...
        match processed {
            ...
            ProcessedPacket::FinalHop(destination, _, payload) => {
                ...
                if dest == our_id {
                    ...
                    if let Ok(module_message) = serde_yaml::from_str::<NetworkWrapper>(std::str::from_utf8(&payload.recover_plaintext().unwrap()).unwrap()) {
    ...
    async fn test_sphinx_packet_3_layers() {
        ...
        // just create a packet with one layer of encryption
        let route = core.create_route(core.get_config().path_length(), None, None, Some(our_id)).await;
