...
impl WebRTCClient{
    ...
    pub async fn msg_receive(&mut self) -> Result<String, String> {
        self.rtc.call(WebRTCMethod::MsgReceive, None).await.map(|s| s.unwrap())
    }
    ...
    pub async fn msg_send(&mut self, s: String) -> Result<(), String> {
        self.rtc.call(WebRTCMethod::MsgSend, Some(s)).await.map(|_| ())
    }
...
mod dummy;
#[cfg(not(target_arch = "wasm32"))]
pub use dummy::WebRTCCaller;

#[cfg(target_arch = "wasm32")]
mod wasm;
#[cfg(target_arch = "wasm32")]
pub use wasm::WebRTCCaller;
