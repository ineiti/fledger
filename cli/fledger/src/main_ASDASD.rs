...
//                     if let Some(gossip) = node.gossip.as_mut() {
...
//                         gossip.update();
//                     } else {
//                         log::error!("Failed to get mutable reference to gossip");
//                     }

//                     let events = node.gossip.as_mut().unwrap().storage.events(Category::LoopixSetup);
...
//                 log::info!("Our ID: {}", our_id);

//                 let node_ids: Vec<_> = setup.all_nodes.iter().map(|node| node.get_id()).collect();
//                 log::info!("Node IDs: {:?}", node_ids);
